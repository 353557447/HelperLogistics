...
//    status==2 根据pay_style==1 时 数量可以增加 ，可以减少
    ...
    public BucketHandleOrderAdapter(Context context, boolean isRecycler) {
        mContext = context;
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        holder.name.setText(mBucketBean.getGname());
        if (isRecycler) {
            holder.deleteBtn.setVisibility(View.VISIBLE);
            holder.image.setVisibility(View.VISIBLE);
            Glide.with(mContext).load(Constant.GOODS_IMAGE_URL + mBucketBean.getPicturl()).into(holder.image);
