...
@BaseViewInject(contentViewId = R.layout.activity_wh_search_order)
public class WhSearchOrderActivity extends BaseActivity {
    @BindView(R.id.rv)
    RecyclerView mRv;
    ...
    private void search() {
        RetrofitUtils.getInstances().create().warehouseSearchOrder(mSearchContent).enqueue(new Callback<Object>() {
            @Override
